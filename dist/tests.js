...
if (is_array_not_array_1.isArray(result) && result[0] === 1 &&
    result.length === 3)
...
if (is_array_not_array_1.isArray(result) && result[0] === 4 &&
    result.length === 3)
...
if (is_array_not_array_1.isArray(result) && result[0] === 3 && result[4] === 7 &&
    result.length === 5)
...
errorTriggered = false;
try {
    result = index_1.getAdjacentToValue({ value: 6, offset: 0, howMany: 2.1 }, arr);
}
catch (e) {
    errorTriggered = true;
}
...
errorTriggered = false;
try {
    result = index_1.getAdjacentToValue({ value: 6, offset: 0, howMany: -1 }, arr);
}
catch (e) {
    errorTriggered = true;
}
